...
const announcements = [
  { title: "Quarter 2 Exams Schedule Released", from: "School Admin", time: "2h ago", preview: "The quarterly examination schedule for all grade levels has been posted...", isNew: true },
  { title: "Science Fair Registration Open", from: "Ms. Cruz - Science", time: "5h ago", preview: "Students interested in joining the Science Fair may now register...", isNew: true },
  { title: "MAPEH Performance Task Reminder", from: "Ms. Dela Cruz - MAPEH", time: "1d ago", preview: "Please submit your dance performance video by Friday...", isNew: false },
];
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                {item.isNew && (
                  <Badge className="text-[8px] font-bold px-1.5 py-0 rounded-full bg-destructive text-destructive-foreground border-0 shrink-0">NEW</Badge>
                )}
              </div>
              <p className="text-[10px] text-muted-foreground font-semibold mb-1">{item.from} • {item.time}</p>
              <p className="text-[11px] text-muted-foreground line-clamp-1">{item.preview}</p>
            </div>
