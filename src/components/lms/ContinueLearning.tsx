...
import { Play, Clock, BookOpenCheck } from "lucide-react";
      ...
      <div className="grid grid-cols-2 gap-2.5">
        {lessons.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className="bg-card rounded-xl border border-border/50 overflow-hidden transition-all duration-200 active:scale-[0.97] cursor-pointer card-shadow hover:shadow-md"
            >
              ...
              <div className="p-2.5">
                <p className="text-[8px] text-primary font-bold uppercase tracking-wider">{item.subject_name}</p>
                <h4 className="text-[11px] font-bold text-foreground leading-tight mt-0.5 line-clamp-2">{item.lesson_title}</h4>
                <p className="text-[8px] text-muted-foreground mt-0.5">{item.chapter}</p>
                <div className="mt-2 flex items-center gap-1.5">
                  <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
                    <div className={`bg-gradient-to-r ${gradient} h-1.5 rounded-full`} style={{ width: `${item.progress || 0}%` }} />
                  </div>
                  <span className="text-[8px] font-bold text-muted-foreground">{item.progress || 0}%</span>
                </div>
              </div>
            </div>
          );
