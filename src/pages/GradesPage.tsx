...
import StudentTermGradesTab from "@/components/lms/StudentTermGradesTab";
import { Progress } from "@/components/ui/progress";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { TrendingDown, Target, Sparkles, ChevronDown, Rocket, CalendarCheck, Inbox, Lock, ListChecks, ClipboardList } from "lucide-react";
import { cn } from "@/lib/utils";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/contexts/AuthContext";
import { useSectionMembership } from "@/hooks/useSectionMembership";
import { remark, remarkColor } from "@/lib/term-grades";
...
const GradesQuizTab = () => {
  ...
  if (loading) {
    return <div className="text-center py-8 text-sm text-muted-foreground">Loading quiz grades…</div>;
  }
  ...
  if (enriched.length === 0) {
    return (
      <div className="text-center py-10">
        ...
        <p className="text-sm font-bold text-foreground">No quiz grades yet</p>
...
const GradesPage = () => {
  ...
  const [gradeTab, setGradeTab] = useState("terms");
      ...
      <div className="min-h-screen bg-background pb-8">
        ...
        <div className="max-w-3xl mx-auto px-4 py-10 text-center">
          <p className="text-sm text-muted-foreground">
            Grades view is available for students. Teachers enter term grades in each subject dashboard → Term Grades.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background pb-8">
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="px-4 space-y-4">
          <div>
            <h2 className="text-base font-bold text-foreground">Grades</h2>
            <p className="text-[11px] text-muted-foreground">Quiz performance and teacher-posted term averages.</p>
          </div>

          <Tabs value={gradeTab} onValueChange={setGradeTab} className="w-full">
            <TabsList className="w-full grid grid-cols-2 h-10 rounded-xl">
              <TabsTrigger value="terms" className="rounded-lg text-xs font-bold gap-1.5">
                <ClipboardList className="h-3.5 w-3.5" /> Term Grades
              </TabsTrigger>
              <TabsTrigger value="quizzes" className="rounded-lg text-xs font-bold gap-1.5">
                <ListChecks className="h-3.5 w-3.5" /> Quiz Scores
              </TabsTrigger>
            </TabsList>
            <TabsContent value="terms" className="mt-4 focus-visible:outline-none">
              <StudentTermGradesTab />
            </TabsContent>
